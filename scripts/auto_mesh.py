...
def extract_ips(response, name):
    ...
    if response.code >= 400:
        print(f"HTTP ERROR CODE: {str(response.code)}")
...
class SimpleParser:
    ...
    def parse_properties(self):
        line = self.next()
        if not line:
            raise Exception(f"Could not parse properties: {self.lines[-1]}")
        properties = {}
        while line.strip() != "}":
            ...
            line = self.next()
            if not line:
                raise Exception(f"Could not find end of properties: {self.lines[-1]}")
        self.position += 1
        return properties
...
class SimpleConfig:
    ...
    def append_connectors(self, connectors):
        ...
        with open(self.filename, "a") as f:
            print("", file=f)
            print(f"# generated by auto-mesh {mode}", file=f)
            for c in connectors:
                c.update(properties)
...
def infer():
    service_name = os.environ.get(
        "QDROUTERD_AUTO_MESH_SERVICE_NAME",
        f'{os.environ.get("APPLICATION_NAME", "amq-interconnect")}-headless',
    )
    ...
    return [
        {
            "role": "inter-router",
            "host": f"{prefix}-{i}.{service_name}.{namespace}.svc.cluster.local",
        }
...
if __name__ == "__main__":
    ...
    if mode and len(sys.argv) > 1:
        try:
            ...
            traceback.print_exc()
            sys.exit(f"Error configuring automesh: {e}")
